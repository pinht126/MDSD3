...
class MDAttention(nn.Module):
    ...
    def __init__(
        ...
        self.added_proj_bias = added_proj_bias
        if self.added_kv_proj_dim is not None:
            self.add_k_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=self.added_proj_bias)
            self.add_v_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=self.added_proj_bias)
            if self.context_pre_only is not None:
                self.add_q_proj = nn.Linear(added_kv_proj_dim, self.inner_dim, bias=self.added_proj_bias)
        
        self.added_proj_bias_clean = added_proj_bias
        if self.added_kv_proj_dim is not None:
            self.to_k_clean = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=self.added_proj_bias_clean) #(JW) bias?
            self.to_v_clean = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=self.added_proj_bias_clean) #(JW) bias?
            if self.context_pre_only is not None:
                self.to_q_clean = nn.Linear(added_kv_proj_dim, self.inner_dim, bias=self.added_proj_bias_clean) #(JW) bias?
        ...
        if self.context_pre_only is not None and not self.context_pre_only:
            ...
            self.to_add_out = nn.Linear(self.inner_dim, self.out_dim, bias=self.out_bias)
            self.to_out_clean = nn.ModuleList([])
            self.to_out_clean.append(nn.Linear(self.inner_dim, self.out_dim, bias=self.out_bias)) #(JW) bias?
        ...
        for _ in range(3):
            controlnet_block = nn.Linear(1536, 1536)
            controlnet_block = zero_module(controlnet_block)
            self.zero_blocks.append(controlnet_block)
        # set attention processor
        # We use the AttnProcessor2_0 by default when torch 2.x is used which uses
        # torch.nn.functional.scaled_dot_product_attention for native Flash/memory_efficient_attention
        # but only if it has the default `scale` argument. TODO remove scale_qk check when we move to torch 2.1
    ...
    def forward(
        ...
        hidden_states: torch.Tensor,
        encoder_hidden_states: Optional[torch.Tensor] = None,
        clean__hidden_states: Optional[torch.Tensor] = None,
        ...
        **cross_attention_kwargs,
        ...
        return self.processor(
            ...
            hidden_states,
            encoder_hidden_states=encoder_hidden_states,
            clean__hidden_states=clean__hidden_states,
            mask=mask,
            scale1=scale1,
            **cross_attention_kwargs,
        )
...
class MDJointAttnProcessor2_0:
    ...
    def __call__(
        ...
        hidden_states: torch.FloatTensor,
        encoder_hidden_states: torch.FloatTensor = None,
        clean_hidden_states: torch.FloatTensor = None,
        ...
        **kwargs,
        ...
        if context_input_ndim == 4:
            batch_size, channel, height, width = encoder_hidden_states.shape
            encoder_hidden_states = encoder_hidden_states.view(batch_size, channel, height * width).transpose(1, 2)
        clean_input_ndim = clean_hidden_states.ndim
        if clean_input_ndim == 4: ##clean
            batch_size, channel, height, width = clean_hidden_states.shape
            clean_hidden_states = clean_hidden_states.view(batch_size, channel, height * width).transpose(1, 2)
        batch_size = encoder_hidden_states.shape[0]
        ...
        encoder_hidden_states_query_proj = encoder_hidden_states_query_proj * mask
        encoder_hidden_states_key_proj = encoder_hidden_states_key_proj * mask
        encoder_hidden_states_value_proj = encoder_hidden_states_value_proj * mask

        #(JW) `clean` projections.
        clean_hidden_states_query_proj = attn.to_q_clean(clean_hidden_states)
        clean_hidden_states_key_proj = attn.to_k_clean(clean_hidden_states)
        clean_hidden_states_value_proj = attn.to_v_clean(clean_hidden_states)
        
        #(JW) image_qkv + zero(clean_qkv) : optinal
        if scale1 > 0:
            scale=scale1
            query = query + scale * attn.zero_blocks[0](clean_hidden_states_query_proj)
            key = key + scale * attn.zero_blocks[1](clean_hidden_states_key_proj)
            value = value + scale * attn.zero_blocks[2](clean_hidden_states_value_proj)    
        ...
        clean_states = F.scaled_dot_product_attention(clean_hidden_states_query_proj, clean_hidden_states_key_proj, clean_hidden_states_value_proj, dropout_p=0.0, is_causal=False)
        clean_states = clean_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
        clean_states = clean_states.to(query.dtype)
        ...
        if not attn.context_pre_only:
            encoder_hidden_states = attn.to_add_out(encoder_hidden_states)
            clean_hidden_states = attn.to_out_clean[0](clean_states)
        ...
        if context_input_ndim == 4:
            encoder_hidden_states = encoder_hidden_states.transpose(-1, -2).reshape(batch_size, channel, height, width)
            clean_hidden_states = clean_hidden_states.transpose(-1, -2).reshape(batch_size, channel, height, width)
        return hidden_states, encoder_hidden_states, clean_hidden_states
