...
class StableDiffusion3MDPipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        clean_image_ = self.prepare_image(
            image=clean_image,
            width=width,
            height=height,
            batch_size=batch_size * num_images_per_prompt,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if isinstance(controlnet_keep[i], list):
                    ...
                    if isinstance(controlnet_cond_scale, list):
                        controlnet_cond_scale = controlnet_cond_scale[0]
                    cond_scale = controlnet_cond_scale * controlnet_keep[i] #(JW)

                noise_pred = self.transformer(
                    hidden_states=latent_model_input,
                    clean__hidden_states=clean_image,
                    timestep=timestep,
                    encoder_hidden_states=prompt_embeds,
                    pooled_projections=pooled_prompt_embeds,
                    joint_attention_kwargs=self.joint_attention_kwargs, #(JW)
                    return_dict=False,
                    mask=mask
                )[0]
        ...
        return StableDiffusion3MDPipeline(images=image)